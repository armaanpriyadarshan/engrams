...
function buildMountScene(
  ...
  return function mountScene(
    ...
    const nodeMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float zoomBrightness = 1.9 - smoothstep(15.0, 50.0, vSize) * 0.7;
    ...
    const edgeMat = new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.32 })
      ...
      if (closest !== state.currentHovered || hoveredEdge !== state.currentHoveredEdge) {
        ...
        if (closest >= 0) {
          ...
          for (let i = 0; i < count; i++) {
            ...
            buffers.fadeTarget[i] = (i === closest || nbs.has(i)) ? 1.0 : (filterOk ? 0.25 : 0.06)
          }
          const i3 = closest * 3
          projVec.set(cp[i3], cp[i3 + 1], cp[i3 + 2]).project(state.camera)
          ...
          for (let i = 0; i < count; i++) {
            const filterOk = !vis || vis[i] === 1
            buffers.fadeTarget[i] = (i === buffers.eSrc[hoveredEdge] || i === buffers.eTgt[hoveredEdge]) ? 1.0 : (filterOk ? 0.25 : 0.06)
          }
        } else {
          const vis = nodeVisibleRef.current
