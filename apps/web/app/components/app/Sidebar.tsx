...
export function Sidebar({ engrams, profile }: { engrams: Engram[]; profile: Profile | null }) {
  ...
  return (
    <aside
      className="shrink-0 border-r border-border bg-surface flex flex-col h-full overflow-hidden transition-[width] duration-300 ease-in-out"
      style={{ width: collapsed ? 40 : 224 }}
    >
      {/* Collapsed view */}
      <div className={`flex flex-col items-center transition-opacity duration-200 ${collapsed ? "opacity-100" : "opacity-0 pointer-events-none absolute"}`}>
        <button
      ...
      {/* Expanded view */}
      <div className={`flex flex-col h-full min-w-[224px] transition-opacity duration-200 ${collapsed ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
